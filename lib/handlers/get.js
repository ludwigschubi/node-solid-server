...
async function handler (req, res, next) {
  ...
  if (requestedType && requestedType.includes('text/html')) {
    const { path: filename } = await ldp.resourceMapper.mapUrlToFile({ url: options, searchIndex: false })
    ...
    if (!isHtmlResource) {
      ...
      //respond to requests in the browser with json-ld
      const data = await translate(stream, baseUri, contentType, 'text/turtle')
