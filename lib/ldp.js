...
class LDP {
  ...
  async get (options, searchIndex = true) {
    const acceptHtml = options.contentType ? options.contentType.split(',').indexOf('text/html') !== -1 : true;
    const getIndex = (options.contentType && !acceptHtml) ? false : searchIndex;
    console.log(options.path, options.contentType, getIndex);
    let path, contentType, stats
    try {
      ({ path, contentType } = await this.resourceMapper.mapUrlToFile({ url: options, searchIndex: getIndex }))
      console.log(path);
